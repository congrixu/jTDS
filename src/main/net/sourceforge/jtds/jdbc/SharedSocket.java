...
class SharedSocket {
	...
	*/
	private Socket createSocketForJDBC3(JtdsConnection connection) throws IOException {
		...
		Socket socket = null;
		String useProxy = DefaultProperties.USE_PROXY;
		String proxyHost = DefaultProperties.PROXY_HOST;
		String proxyPort = DefaultProperties.PROXY_PORT;
		if ("true".equals(useProxy) && proxyHost != null && proxyHost.trim().length() > 0 && proxyPort != null
	...
	*/
		...
		synchronized (_VirtualSockets) {
			...
			if (responseOwner != vsock)
				throw new IOException(
						"Stream " + vsock.id + " is trying to read data that belongs to stream " + responseOwner.id);
	...
	private void enqueueInput(VirtualSocket vsock, byte[] buffer) throws IOException {
		...
		//
		if (globalMemUsage + buffer.length > memoryBudget && vsock.pktQueue.size() >= minMemPkts && !securityViolation
	...
	private byte[] readPacket(byte buffer[]) throws IOException {
		...
		if (packetType != TdsCore.LOGIN_PKT && packetType != TdsCore.QUERY_PKT && packetType != TdsCore.SYBQUERY_PKT // required to connect IBM/Netcool Omnibus, see patch [1844846]
		...
		//
		if (++packetCount == 1 && serverType == Driver.SQLSERVER && "NTLMSSP".equals(new String(buffer, 11, 7))) {
			buffer[1] = 1;
		}
