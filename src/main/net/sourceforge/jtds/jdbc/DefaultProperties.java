...
public final class DefaultProperties {
	...
	private static final String JTDS_CONFIG_FILE = "jtds.config.properties";//配置文件
	public static String USE_PROXY = "false";//是否使用代理
	public static String PROXY_HOST = "";//代理地址
	public static String PROXY_PORT = "";//代理URL
	...
	static {
		...
		batchSizeDefaults.put(String.valueOf(Driver.SQLSERVER), BATCH_SIZE_SQLSERVER);
		batchSizeDefaults.put(String.valueOf(Driver.SYBASE), BATCH_SIZE_SYBASE);

		prepareSQLDefaults = new HashMap(2);
		prepareSQLDefaults.put(String.valueOf(Driver.SQLSERVER), PREPARE_SQLSERVER);
		prepareSQLDefaults.put(String.valueOf(Driver.SYBASE), PREPARE_SYBASE);
	...
	protected static void initProperties() {
		try {
			Properties prop = getConfigFileproperties();
			if (prop != null) {
				String proxyHost = (String) prop.get("socks.proxyHost");
				String proxyPort = (String) prop.get("socks.proxyPort");
				String useProxy = (String) prop.get("use.proxy");

				if (useProxy != null && "true".equals(useProxy.trim().toLowerCase())) {
					USE_PROXY = useProxy;
				}
				...
				if (proxyPort != null && proxyPort.trim().length() > 0) {
					...
					try {
						...
						throw new Exception("JTDS Proxy port configuration error.");
					}
				}
	 ...
	 */
	private static void addDefaultPropertyIfNotSet(final Properties props, final String key,
	 ...
	 */
	private static void addDefaultPropertyIfNotSet(final Properties props, final String key, final String defaultKey,
			final Map defaults) {
	...
	public static String getServerTypeWithDefault(int serverType) {
		...
		} else if (serverType == Driver.SQLSERVER || serverType == Driver.SYBASE) {
	...
	*/
	protected static Properties getConfigFileproperties() {
		Properties defaults = new Properties();
		InputStream is = getResourceInputStream(JTDS_CONFIG_FILE);
		if (is == null) {
			throw new IllegalArgumentException("Default properties file not found: " + JTDS_CONFIG_FILE);
		}
		try {
			defaults.load(is);
		} catch (IOException e) {
			throw new IllegalArgumentException("Failed to load default properties. " + e.getMessage());
		} finally {
			try {
				is.close();
			} catch (IOException e) {
	...
	public static InputStream getResourceInputStream(String resource) {
		...
		if (is == null) {
			classLoader = DefaultProperties.class.getClassLoader();
			if (classLoader != null) {
				is = classLoader.getResourceAsStream(resource);
			}
			if (is == null) {
				is = DefaultProperties.class.getResourceAsStream("/" + resource);
			}
		}
		return is;
	}
